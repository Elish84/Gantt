 ...
 *   tasks: [{id, topicId, title, desc, start, end, status}]
 * }
 */
...
let selectedStatuses = new Set(["todo","in_progress","done"]); // filter by status
...
function setUIEnabled(enabled){
  const ids = [
    ...
    "taskStatus","btnStatusAll","btnStatusClear"
  ];
  ids.forEach(id => {
    const e = el(id);
...
async function loadProject(projectId){
  ...
  // restore filters (topics + statuses) if exist
  restoreSelection();
  restoreStatusSelection();
  persistSelection();
  persistStatusSelection();
  renderAll();
  setProjectPill();
}
...
function persistStatusSelection(){
  try{
    if(!project || !currentProjectId) return;
    localStorage.setItem(`gantt_stat_${currentProjectId}`, JSON.stringify([...selectedStatuses]));
  }catch(e){}
}
function restoreStatusSelection(){
  try{
    if(!project || !currentProjectId) return;
    const raw = localStorage.getItem(`gantt_stat_${currentProjectId}`);
    if(!raw) return;
    const arr = JSON.parse(raw);
    if(Array.isArray(arr) && arr.length){
      selectedStatuses = new Set(arr);
    }
  }catch(e){}
}

function refreshStatusChips(){
  const host = el("statusChips");
  if(!host) return;
  host.innerHTML = "";
  ...
  ["todo","in_progress","done"].forEach(st => {
    const label = document.createElement("label");
    label.className = "chip";
    if(selectedStatuses.has(st)) label.classList.add("active");

    const cb = document.createElement("input");
    cb.type = "checkbox";
    cb.checked = selectedStatuses.has(st);
    cb.addEventListener("change", () => {
      if(cb.checked) selectedStatuses.add(st);
    ...
    const txt = document.createElement("span");
    txt.textContent = STATUS_LABELS[st] || st;
...
el("btnStatusAll").addEventListener("click", () => {
  selectedStatuses = new Set(["todo","in_progress","done"]);
  persistStatusSelection();
  refreshStatusChips();
  renderAll();
});
el("btnStatusClear").addEventListener("click", () => {
  selectedStatuses = new Set(); // show none
  persistStatusSelection();
  refreshStatusChips();
  renderAll();
});
...
el("btnAddTask").addEventListener("click", () => {
  ...
  const status = el("taskStatus").value || "todo";
  const start = el("taskStart").value;
...
function renderTasksTable(){
  ...
  const rows = [...project.tasks]
    .filter(t => selectedStatuses.has((t.status || "todo")))
  ...
  rows.forEach(task => {
    ...
    tr.innerHTML = `
      ...
      <td><span class="status-pill ${escapeAttr(task.status||'todo')}">${escapeHTML(STATUS_LABELS[task.status]||task.status||'')}</span></td>
...
const CSV_COLUMNS = ["topic","title","status","start","end","duration_days","desc"];
...
function buildCSV(proj){
  ...
  proj.tasks.forEach(t => {
    ...
    const vals = [
      topic.name,
      t.title,
      (STATUS_LABELS[t.status] || t.status || "todo"),
      t.start,
      t.end,
      dur,
      t.desc || ""
...
function parseCSV(text){
  ...
  return rows.map(r => ({
    ...
    status: (get(r,"status") || get(r,"סטטוס") || "").trim(),
...
function applyImportedTasks(imported){
  ...
  imported.forEach(row => {
    ...
    if(status === "בוצע" || status === "done") status = "done";
    else if(status === "בתהליך" || status === "in progress" || status === "in_progress") status = "in_progress";
    else if(status === "טרם בוצע" || status === "todo" || status === "not started" || status === "not_started") status = "todo";
    else status = "todo";
...
function buildTaskGroups(){
  ...
  project.tasks.forEach(task => {
    if(!selectedStatuses.has((task.status || "todo"))) return;
    ...
    tasks2.forEach(task => {
      ...
      if(e > s){
        ...
        row.innerHTML = `
          ...
          <div class="pin drag-handle" data-task-id="${escapeAttr(task.id)}" data-handle="start" data-start="${s}" title="גרור כדי לשנות תאריך התחלה"></div>
          <div class="date-tag" data-start="${s}">${escapeHTML(tagStart)}</div>

          <div class="range-line" data-task-id="${escapeAttr(task.id)}" data-start="${s}" data-end="${e}"></div>

          <div class="pin drag-handle" data-task-id="${escapeAttr(task.id)}" data-handle="end" data-end="${e}" title="גרור כדי לשנות תאריך סיום"></div>
        ...
        row.innerHTML = `
          <div class="connector" data-start="${s}"></div>
          <div class="pin drag-handle" data-task-id="${escapeAttr(task.id)}" data-handle="start" data-start="${s}" title="גרור כדי לשנות תאריך"></div>
          <div class="date-tag" data-start="${s}">${escapeHTML(tagStart)}</div>
        `;
      }
...
/* =========================
   Gantt drag: move start/end pins on timeline (updates task dates)
========================= */
let _dragState = null;

function dayIdxFromClientX(clientX){
  const tl = el("timeline");
  if(!tl) return 0;
  const rect = tl.getBoundingClientRect();
  const x = clientX - rect.left;
  const i = Math.round(((totalW() - x) / pxDay) - 0.5);
  return clamp(i, 0, daysCount()-1);
}

function syncTaskFormIfEditing(taskId){
  if(!taskId || !editingTaskId || editingTaskId !== taskId) return;
  const t = project?.tasks?.find(x => x.id === taskId);
  if(!t) return;
...
function applyDragToDOM(taskId, sIdx, eIdx){
  // update all DOM pieces in the relevant row
  const row = document.querySelector(`.date-col .row[data-task-id="${cssEscape(taskId)}"]`);
  if(!row) return;

  const conn = row.querySelector(".connector[data-start]");
  const pinS = row.querySelector('.pin[data-handle="start"]');
  const pinE = row.querySelector('.pin[data-handle="end"]');
  const tagS = row.querySelector(".date-tag[data-start]");
  const tagE = row.querySelector(".date-tag[data-end]");
  const range = row.querySelector(".range-line");

  if(conn) conn.setAttribute("data-start", String(sIdx));
  if(pinS) pinS.setAttribute("data-start", String(sIdx));
  if(tagS){
    tagS.setAttribute("data-start", String(sIdx));
    tagS.textContent = formatDDMM(DATES[sIdx]);
  }
  if(range){
    range.setAttribute("data-start", String(sIdx));
    range.setAttribute("data-end", String(eIdx));
  }
  if(pinE){
    pinE.setAttribute("data-end", String(eIdx));
  }
  if(tagE){
    tagE.setAttribute("data-end", String(eIdx));
    tagE.textContent = formatDDMM(DATES[eIdx]);
  }
}

function onDragMove(ev){
  if(!_dragState || !project) return;
  const idx = dayIdxFromClientX(ev.clientX);

  const task = project.tasks.find(t => t.id === _dragState.taskId);
  if(!task) return;

  let sIdx = clamp(DATES.indexOf(task.start), 0, daysCount()-1);
  let eIdx = clamp(DATES.indexOf(task.end), 0, daysCount()-1);

  if(_dragState.handle === "start"){
    sIdx = idx;
  ...
  task.start = DATES[sIdx];
  task.end   = DATES[eIdx];

  applyDragToDOM(task.id, sIdx, eIdx);
  positionMarkers();
  drawLinks();
  syncTaskFormIfEditing(task.id);
}

function onDragEnd(ev){
  if(!_dragState) return;
  document.body.classList.remove("dragging");
  // re-render to ensure ordering, range validation, and tags are consistent
  renderAll();
  scheduleAutosave();
  _dragState = null;
}
...
function initDragHandles(){
  const host = el("dateCol");
  if(!host) return;

  host.addEventListener("pointerdown", (ev) => {
    const pin = ev.target?.closest?.(".pin.drag-handle");
    if(!pin || !project) return;

    const taskId = pin.getAttribute("data-task-id");
    const handle = pin.getAttribute("data-handle"); // start/end
    if(!taskId || !handle) return;

    _dragState = { taskId, handle };
...
/* =========================
   View mode: hide/show creation panels (topics + task editor)
========================= */
const VIEWMODE_KEY = "gantt_viewmode_v2";

function setViewMode(on){
  document.body.classList.toggle("view-mode", !!on);
  const b = el("btnTogglePanels");
  if(b){
    b.textContent = on ? "צא מתצוגה" : "מצב תצוגה";
  }
  try{ localStorage.setItem(VIEWMODE_KEY, on ? "1" : "0"); }catch(e){}
}

function initViewMode(){
  let on = false;
  try{ on = localStorage.getItem(VIEWMODE_KEY) === "1"; }catch(e){}
  setViewMode(on);

  const b = el("btnTogglePanels");
  if(b){
    b.addEventListener("click", () => setViewMode(!document.body.classList.contains("view-mode")));
  }
}
