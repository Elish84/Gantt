...
// Sync vertical scrolling between label column and timeline column
(function attachGanttScrollSync(){
  const dateCol = el("dateCol");
  const labelCol = el("labelCol");
  if(!dateCol || !labelCol) return;
  let lock = false;

  dateCol.addEventListener("scroll", () => {
    if(lock) return;
    lock = true;
    labelCol.scrollTop = dateCol.scrollTop;
    lock = false;
    drawLinks(); // keep connectors aligned while scrolling
  }, {passive:true});

  labelCol.addEventListener("scroll", () => {
    if(lock) return;
    lock = true;
    dateCol.scrollTop = labelCol.scrollTop;
    lock = false;
    drawLinks();
  }, {passive:true});
})();
