...
function toISODate(d){
  // Accept Date object or ISO string, return YYYY-MM-DD
  if(typeof d === "string") return d; // Already ISO format
...
function parseISODate(s){
  ...
  if(!s) return null;
  if(s instanceof Date) return s; // Already a Date
...
function addDays(d, n){
  // Accept Date or ISO string, return ISO string
  const dt = parseISODate(d);
  if(!dt) return null;
  const x = new Date(dt.getFullYear(), dt.getMonth(), dt.getDate(), 12, 0, 0, 0);
  x.setDate(x.getDate() + n);
  return toISODate(x);
}

function diffDays(a,b){
  // Whole days between two dates (a..b), accept Date or ISO string
  const aa = parseISODate(a);
  const bb = parseISODate(b);
  if(!aa || !bb) return 0;
  const aaMid = new Date(aa.getFullYear(), aa.getMonth(), aa.getDate(), 12, 0, 0, 0);
  const bbMid = new Date(bb.getFullYear(), bb.getMonth(), bb.getDate(), 12, 0, 0, 0);
  const ms = bbMid - aaMid;
...
// RTL positioning: rightmost date (index 0) appears at right edge
...
/* Draw dashed links from label boundary to each start pin */
let drawLinksTimeout = null;
function drawLinks(){
  // Debounce to avoid excessive redraws
  if(drawLinksTimeout) clearTimeout(drawLinksTimeout);
  drawLinksTimeout = setTimeout(_drawLinksImmediate, 16); // ~60fps
}

function _drawLinksImmediate(){
  ...
  labelBlocks.forEach(lb => {
    ...
    const dateRows = db.querySelectorAll(".rows > .gantt-row");
    dateRows.forEach(dr => {
