...
async function scheduleSave(reason=""){
  if(!currentUser || !project || !currentProjectId) {
    console.warn("scheduleSave skipped (missing user/project/id)", {reason, currentUser:!!currentUser, project:!!project, currentProjectId});
    return;
  }
  ...
  saveTimer = setTimeout(async () => {
    try{
      ...
      alert("שמירה ל-DB נכשלה: " + (e?.message || e));
    }
  }, 50);
}
...
function setUIEnabled(enabled){
  const ids = [
    ...
    "topicName","topicColor","btnAddTopic","btnUpdateTopic","btnDeleteTopic","topicSelect",
...
async function loadProject(projectId){
  ...
  // default selection = all topics visible (or restore last selection)
  selectedTopicIds = new Set(project.topics.map(t=>t.id));
  restoreSelection();
  project.topics.forEach(t=>selectedTopicIds.add(t.id));
  persistSelection();
  renderAll();
  setProjectPill();
}
...
async function saveProject(){
  console.debug("saveProject()", {uid: currentUser?.uid, projectId: currentProjectId});
