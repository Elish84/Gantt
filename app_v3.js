...
function normalizeStatus(s){
  s = String(s||"").trim();
  if(s === STATUS.DONE || s === STATUS.IN_PROGRESS || s === STATUS.NOT_STARTED) return s;
  // legacy / hebrew labels
  if(s === "בוצע") return STATUS.DONE;
  if(s === "בתהליך") return STATUS.IN_PROGRESS;
  if(s === "טרם בוצע") return STATUS.NOT_STARTED;
  return STATUS.NOT_STARTED;
}
function statusLabel(s){ return STATUS_LABELS[normalizeStatus(s)] || STATUS_LABELS[STATUS.NOT_STARTED]; }
function statusSelectHTML(value, cls=""){
  const v = normalizeStatus(value);
  return `
    <select class="sel ${cls}" data-role="statusSelect">
      ${STATUS_ORDER.map(opt => `<option value="${opt}" ${opt===v?"selected":""}>${STATUS_LABELS[opt]}</option>`).join("")}
    </select>
  `;
}
...
let selectedTopicIds = new Set(); // topic filter
let selectedStatuses = new Set(STATUS_ORDER); // status filter (default: all)

// Table filtering/sorting
let tableFilters = { topic:"", title:"", status:"", start:"", end:"", desc:"" };
let tableSort = { key:"start", dir: 1 }; // dir: 1 asc, -1 desc


let viewMode = localStorage.getItem("gantt_view_mode") === "1";
function applyViewMode(){
  document.body.classList.toggle("view-mode", viewMode);
  const b = el("btnToggleView");
  if(b) b.textContent = viewMode ? "הצג ניהול" : "מצב תצוגה";
}
function toggleViewMode(){
  viewMode = !viewMode;
  localStorage.setItem("gantt_view_mode", viewMode ? "1":"0");
  applyViewMode();
}
...
function setUIEnabled(enabled){
  const ids = [
    ...
    "taskTopic","taskTitle","taskDesc","taskStart","taskEnd","taskStatus","taskDuration",
    "btnAddTask","btnUpdateTask","btnClearTaskForm",
    "btnExportCSV","btnImportCSV","btnToggleView"
  ];
  ids.forEach(id => {
    const e = el(id);
...
function setStatusFilter(mode){
  // mode: "all" | "not_started" | "in_progress" | "done"
  if(mode === "all"){
    selectedStatuses = new Set(STATUS_ORDER);
  }else{
    selectedStatuses = new Set([normalizeStatus(mode)]);
  }
...
function bindStatusFilters(){
  ...
  host.addEventListener("click", (e)=>{
    ...
    if(!b) return;
    setStatusFilter(b.getAttribute("data-status"));
  });
  refreshStatusFilterUI();
}
...
function clearTaskForm(){
  ...
  el("taskStart").value = "";
  el("taskEnd").value = "";
  if(el("taskStatus")) el("taskStatus").value = STATUS.NOT_STARTED;
...
function startEditTask(taskId){
  ...
  if(el("taskStatus")) el("taskStatus").value = normalizeStatus(t.status);
...
function bindTableControls(){
  const tbl = el("tasksTable");
  if(!tbl || tbl.__bound) return;
  tbl.__bound = true;

  // Filters row inputs (data-filter)
  tbl.addEventListener("input", (e)=>{
    const inp = e.target.closest("[data-filter]");
    if(!inp) return;
    const k = inp.getAttribute("data-filter");
    tableFilters[k] = inp.value || "";
    renderTasksTable();
  });

  // Sort by clicking headers (data-sort)
  tbl.addEventListener("click", (e)=>{
    ...
    // visual indicator
    tbl.querySelectorAll("th[data-sort]").forEach(x=>x.classList.remove("sort-asc","sort-desc"));
...
function renderTasksTable(){
  ...
  // Base filters (topic + status) are shared with timeline
  let rows = getBaseFilteredTasks();

  // Table column filters
  const f = tableFilters;
  const includes = (a,b) => String(a||"").toLowerCase().includes(String(b||"").toLowerCase());
  if(f.topic)  rows = rows.filter(t => includes((topicsMap.get(t.topicId)||DEFAULT_TOPIC).name, f.topic));
  if(f.title)  rows = rows.filter(t => includes(t.title, f.title));
  if(f.desc)   rows = rows.filter(t => includes(t.desc, f.desc));
  if(f.status) rows = rows.filter(t => normalizeStatus(t.status) === normalizeStatus(f.status));
  if(f.start)  rows = rows.filter(t => String(t.start||"") >= String(f.start));
  if(f.end)    rows = rows.filter(t => String(t.end||"") <= String(f.end));

  // Sorting
  const key = tableSort.key;
  const dir = tableSort.dir;
  const getKey = (t) => {
    if(key==="topic") return (topicsMap.get(t.topicId)||DEFAULT_TOPIC).name || "";
    if(key==="status") return STATUS_ORDER.indexOf(normalizeStatus(t.status));
    return String(t[key]||"");
  };
  rows = [...rows].sort((a,b) => {
  ...
  rows.forEach(task => {
    ...
    tr.innerHTML = `
      ...
      <td>${statusSelectHTML(task.status, "status-inline")}</td>
    ...
    // Inline status update (no edit dialog)
    const sel = tr.querySelector('[data-role="statusSelect"]');
    sel.addEventListener("change", async (e) => {
      const newStatus = normalizeStatus(e.target.value);
      task.status = newStatus;
      await saveProjectDebounced(); // persists to firestore
      renderAll(); // refresh timeline + table (so filters apply)
    });
...
function getBaseFilteredTasks(){
  if(!project) return [];
  // topic filter
  let out = project.tasks.filter(t => selectedTopicIds.has(String(t.topicId||DEFAULT_TOPIC.id)));
  // status filter
  out = out.filter(t => selectedStatuses.has(normalizeStatus(t.status)));
  return out;
}
...
function indexFromClientX(clientX){
  ...
  const x = clientX - rect.left; // 0..totalW
  const total = totalW();
  // convert x to index in RTL (see centerX formula)
  let idx = Math.round((total - x)/pxDay - 0.5);
  idx = clamp(idx, 0, daysCount()-1);
  return idx;
}

function enablePinDrag(){
  const host = el("dateBlocks");
  if(!host) return;

  // prevent double-binding
  if(host.__pinDragBound) return;
  host.__pinDragBound = true;

  let dragging = null; // {taskId, which:"start"|"end"}
  const onMove = (ev) => {
    if(!dragging) return;
    const clientX = (ev.touches && ev.touches[0]) ? ev.touches[0].clientX : ev.clientX;
    const idx = indexFromClientX(clientX);

    const task = project.tasks.find(t => t.id === dragging.taskId);
    if(!task) return;

    let sIdx = DATES.indexOf(task.start);
    let eIdx = DATES.indexOf(task.end);
    if(sIdx < 0) sIdx = idx;
    if(eIdx < 0) eIdx = idx;
    ...
    if(dragging.which === "start"){
      ...
      eIdx = idx;
      if(eIdx < sIdx) sIdx = eIdx;
    }

    task.start = DATES[sIdx];
    task.end   = DATES[eIdx];

    // re-render blocks quickly
    renderBlocks();
    updateGridLayout();
    drawLinks();
  };
  ...
  host.addEventListener("pointerdown", (ev) => {
    ...
    ev.preventDefault();
    const which = pin.hasAttribute("data-end") ? "end" : "start";
    dragging = { taskId, which };
